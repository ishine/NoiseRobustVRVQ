...
## Test loader
AudioLoader = argbind.bind(AudioLoader_EARS_Piared, "test")
...
# @timer()
@torch.no_grad()
def test_loop(signal_clean, signal_noisy, state, save_figs=None, cal_visqol=True):
    ## samples: AudioSignal, (1, 1, T)
    assert signal_clean.shape[0]==signal_noisy.shape[0]==1
    
    st = time.time()
    state.generator.eval()
    device = state.generator.device
    signal_clean = signal_clean.to(device)
    signal_noisy = signal_noisy.to(device)
    ...
    if save_figs:
        ...
        for jj, level in enumerate(level_list_figs):
            out_lev = state.generator(audio_data_noisy=audio_noisy,
                                    audio_data_clean=None,
                                    sample_rate=sr,
                                    n_quantizers=state.generator.n_codebooks+1,
                                    level=level)
    ...
    for i in range(n_codebooks):
        ...
        signal_noisy = signal_noisy[..., :length]
    ...
    if state.generator.model_type == "VBR":
        ...
        out_enc, fmaps = state.generator.encoder(x_noisy=audio_noisy_proc, x_gt=None)
        ...
        for level in level_list:
            ...
            signal_noisy = signal_noisy[..., :length]
            # recons = recons[..., :signal_noisy.audio_data.shape[-1]]
            recons = AudioSignal(recons, signal_noisy.sample_rate)
...
def eval(
    ...
    for idx in tqdm(range(start_idx, num_test_set),
        ...
        signal_noisy = items["signal_noisy"]
